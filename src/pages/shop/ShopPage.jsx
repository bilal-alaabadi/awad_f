...
import imge from "../../assets/تعديل-البنر.png";

const filters = {
  categories: ['الكل', 'العطور', 'بخور' ,'أعواد','ستاندات للأعواد']
};
...
const ShopPage = () => {
  ...
  if (isLoading) return <div className="text-center py-8 text-[#8B4A2C]">جاري تحميل المنتجات...</div>;
        ...
        <div className='flex flex-col md:flex-row md:gap-8 gap-6'>
          ...
          <div className='md:w-1/4'>
            <button
              ...
              className='md:hidden w-full bg-[#8B4A2C] text-white py-2 px-4 rounded-md mb-4 flex items-center justify-between  transition-colors'
            >
          ...
          <div className='md:w-3/4'>
            <div className='flex justify-between items-center mb-6'>
              <h3 className='text-lg font-medium text-[#8B4A2C]'>
            ...
            {products.length > 0 ? (
              <>
                ...
                {totalPages > 1 && (
                  <div className='mt-8 flex flex-col sm:flex-row items-center justify-between gap-4'>
                    <div className="text-sm text-[#8B4A2C]">
                      ...
                      <button
                        ...
                        className={`px-4 py-2 rounded-md border transition-colors ${
                          currentPage === 1
                            ...
                            : 'border-[#8B4A2C] text-[#8B4A2C] hover:bg-black hover:text-white'
                        }`}
                      >
                        السابق
                      </button>
                          ...
                          return (
                            <button
                              ...
                              className={`w-10 h-10 flex items-center justify-center rounded-md border transition-colors ${
                                active
                                  ? 'bg-[#8B4A2C] text-white border-[#8B4A2C] '
                                  : 'border-[#8B4A2C] text-[#8B4A2C] bg-white  hover:text-white'
                              }`}
                            >
                              {index + 1}
                            </button>
                      ...
                      <button
                        ...
                        className={`px-4 py-2 rounded-md border transition-colors ${
                          currentPage === totalPages
                            ? 'bg-gray-200 text-gray-500 cursor-not-allowed border-gray-200'
                            : 'border-[#8B4A2C] text-[#8B4A2C]  hover:text-white'
                        }`}
                      >
                        التالي
                      </button>
              ...
              <div className="text-center py-12 bg-white rounded-lg shadow-sm">
                <p className="text-lg text-[#8B4A2C]">لا توجد منتجات متاحة حسب الفلتر المحدد</p>
                <button
                  onClick={clearFilters}
                  className="mt-4 px-4 py-2 bg-[#8B4A2C] text-white rounded-md  transition-colors"
                >
                  عرض جميع المنتجات
                </button>
              </div>
